...
router.post('/find_control_logs', urlencodedParser, function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("control_logs").find(whereStr).toArray(function (err, result) { // 返回集合中所有数据   
            if (err) throw err;
            console.log(result);
            db.close();
            res.send(result);
...
router.post('/find_control_logs_All', urlencodedParser, function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db("api");//数据库名字
        dbo.collection("control_logs").find({}).toArray(function (err, result) { // 返回集合中所有数据   
            if (err) throw err;
            console.log(result);
            db.close();
            res.send(result);
...
router.post('/control', urlencodedParser, function (req, res) {
    console.log("control_id: " + req.body.control_id + ",sensor_id:" + req.body.id + ",intensity" + req.body.intensity);
    //res.send('post successfully!'+req.body);

    MongoClient.connect(url, function (err, db) {                //插入操作数据库
        ...
        //update control数据集
        var whereStr = { "control_id": req.body.control_id, "sensor_id": req.body.sensor_id };  // 查询条件
        var updaapir = { $set: { "intensity": req.body.intensity } };

        dbo.collection("control").updateOne(whereStr, updaapir, function (err, res) {
            if (err) throw err;
            console.log("All更新成功");
            db.close();
        });
        ...
        var query_control_logs = { "control_id": req.body.control_id, "sensor_id": req.body.sensor_id, "intensity": req.body.intensity }
        dbo.collection("control_logs").insertOne(query_control_logs, function (err, result) {   //插入操作记录数据库
            db.close;
...
router.get('/sensor/control', function (req, res) {
            ...
            case 0:
                ...
                MongoClient.connect(url, function (err, db) {
                    ...
                    dbo.collection("control").findOne(whereStr, function (err, result) {
                        ...
                        dbo.collection("sensor").updateOne(whereStr, updaapir, function (err, result) {
                            if (err) throw err;
                            console.log("更改sensor数据库: " + "temperature： " + temperature);
                        ...
                        if (temperature > 38) {
                            ...
                            MongoClient.connect(url, function (err, db) {
                                ...
                                var whereStr = { "id": sensor__id, "warnMsg": warnMsg1, "name": "temperature", num: temperature };

                                dbo.collection("warning").insertOne(whereStr, function (err, result) {
                        ...
                        dbo.collection("control").findOne(whereStr, function (err, result) {
                            ...
                            if (humidity > 70) {
                                ...
                                MongoClient.connect(url, function (err, db) {
                                    ...
                                    var whereStr = { "id": sensor__id, "warnMsg": warnMsg1, "name": "humidity", num: humidity };
                    ...
                    dbo.collection("control").findOne(whereStr, function (err, result) {
                        ...
                        if (light > 28) {
                            ...
                            MongoClient.connect(url, function (err, db) {
                                ...
                                var whereStr = { "id": sensor__id, "warnMsg": warnMsg3, "name": "light", num: light };  // 查询条件，不需要条件时 写{}即可
                    ...
                    dbo.collection("control").findOne(whereStr, function (err, result) {
                        ...
                        if (ph > 8) {
                            ...
                            MongoClient.connect(url, function (err, db) {
                                ...
                                var whereStr = { "id": sensor__id, "warnMsg": warnMsg4, "name": "ph", num: ph };  // 查询条件，不需要条件时 写{}即可
                    ...
                    dbo.collection("control").findOne(whereStr, function (err, result) {
                        ...
                        if (wind > 4) {
                            ...
                            MongoClient.connect(url, function (err, db) {
                                ...
                                var whereStr = { "id": sensor__id, "warnMsg": warnMsg5, "name": "wind", num: wind };  // 查询条件，不需要条件时 写{}即可
            ...
            case 5:
                ...
                MongoClient.connect(url, function (err, db) {
                    ...
                    dbo.collection("sensor").findOne(whereStr, function (err, result) {
                        ...
                        var FindStr = { "sensor_id": sensor__id, "temperature": temperature, "humidity": humidity, "light": light, "wind": wind, "ph": ph }
                        dbo.collection("sensor_logs").insertOne(FindStr, function (err, result) {
                            var whereStr = {};

                            //查询传感器记录所有数据
                            dbo.collection("sensor_logs").find(whereStr).toArray(function (err, result) {
    ...
    //     var whereStr = { "control_id": "1", "sensor_id": sensor__id };  // 查询条件，不需要条件时 写{}即可
    ...
    //         humidity_intensity = parseInt(later_intensity);
    //         console.log(humidity_intensity);
    //         humidity = GetRandomNum(0, 100) + humidity_intensity;
    //         console.log("更改湿度为" + humidity)
    //         db.close();
    //         //湿度报警
    //         if (humidity > 70) {
    //             warnMsg2 = "湿度过高";
    ...
    //                 var whereStr = { "id": sensor__id, "warnMsg": warnMsg2, "name": "humidity", num: humidity };  // 查询条件，不需要条件时 写{}即可
    ...
    //         whereStr = { "sensor_id": sensor__id };  // 查询条件，不需要条件时 写{}即可
    //         var updaapir = { $set: { "light": light } };
    ...
    //     dbo.collection("sensor_logs").insertOne(whereStr, function (err, result) {
    ...
    //     var whereStr = { "sensor_id": sensor__id };  // 查询条件，不需要条件时 写{}即可
    //     var updaapir = { $set: { "temperature": temperature, "humidity": humidity, "light": light, "wind": wind, "ph": ph } };
...
router.get('/warn', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("warning").find(whereStr).toArray(function (err, result) {
...
router.get('/sensor/find', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("sensor").find(whereStr).toArray(function (err, result) {
...
router.post("/sensor/new", function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        if (err) throw err
        var dbo = db.db("api");
        var mysensor = { "sensor_id": sensor_id, "name": name, "status": status, "power": power, "place": place, "temperature": temperature, "humidity": humidity, "light": light, "wind": wind, "ph": ph }
